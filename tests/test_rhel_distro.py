...
import distro
from infi.os_info import distro_is_rhel_based, platform
...
class PlatformStringTestCase(unittest.TestCase):
    ...
    def test_platform_string(self, test_subject):
        ...
        self.assertEquals(expected, distro_is_rhel_based(FakePlatformModule(**copy_subject)))


class TestGetPlatformString(unittest.TestCase):
    """A deeper test of all the pipe of distro_is_rhel_based"""
    @unittest.parameters.iterate('test_subject', test_subjects)
    def test_distro_is_rhel_based(self, test_subject):
        with patch("distro.id") as distro_id,\
            ...
            self.assertEquals(expected, distro_is_rhel_based())
